...
contract sampleLoto is ERC721Token, Ownable {
  ...
  event RandomNumber(uint blockIdx, uint gasprice, uint gaslimit, uint specialNumber, uint randNumber);
  ...
  */
  ...
  function endBet() public onlyOwner{
    ...
    bytes32 _specialNumber = sha256(tx.gasprice, block.gaslimit);
    uint specialNumber = uint(_specialNumber);
    while ( idx < 100 ) {
      /* 擬似乱数の生成 */
      bytes32 _seed = sha256(block.blockhash(blockIdx--), now, tx.gasprice, block.gaslimit, specialNumber);
      uint randNumber = ( uint(_seed) % 100 );
      emit RandomNumber(blockIdx, tx.gasprice, block.gaslimit, specialNumber, randNumber);
      idx++;
    }
  }
}