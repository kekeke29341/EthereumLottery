...
contract sampleLoto is ERC721Token, Ownable {
  ...
  /* コイン名 */
  string public name = "millionaire Loto";
  /* コインシンボル */
  string public symbol = "M-loto";
  /* フラグメント */
  bool public betClosed = false;

  uint private FirstPrize;
  uint private SecondPrize;
  uint private ThirdPrize;
  uint private FourthPrize;
  uint private FifthPrize;
  
  
  /* 賞金配当者の数を代入する変数 */
  uint totalWinner = 0;
  /* 擬似乱数生成の際のblock numberを格納する変数 */
  ...
  //event Winner(uint totalWinner, uint FirstPrize, uint SecondPrize, uint ThirdPrize, uint FourthPrize, uint SpecialPrize);
  ...
  event resultReport(uint FirstPrize, uint SecondPrize, uint ThirdPrize, uint FourthPrize, uint SpecialPrize, uint totalBetPrice, uint totalPrize);
  event ethTransfer(address from_, address to_, uint amount_);

  //コンストラクタ
  function sampleLoto() public ERC721Token(name, symbol) {}

  function buyLoto () external payable {
  ...
  */
  ...
  function endBet() external onlyOwner{
    ...
    emit resultReport(totalWinner, FirstPrize, SecondPrize, ThirdPrize, FourthPrize, FifthPrize, totalBetPrice, totalPrize);
  ...
  function determineWinners() internal returns (uint) {
    ...
    /* 各々のgradeの当選者の人数の決定 */
    FirstPrize = 1;
    SecondPrize = (totalTicket).mul(10).div(100).div(10);
    ThirdPrize = (totalTicket).mul(15).div(100).div(5);
    FourthPrize = (totalTicket).mul(30).div(100).div(3);
    FifthPrize = (totalTicket).mul(30).div(100).div(1);

    /* 勝者の人数（合計） */
    totalWinner = FirstPrize + SecondPrize + ThirdPrize + FourthPrize + FifthPrize;
    //emit Winner(totalWinner, FirstPrize, SecondPrize, ThirdPrize, FourthPrize, FifthPrize);
    ...
    while ( idx < totalWinner ) {
      /* 擬似乱数の生成 */
      uint specialNumber = uint(sha256(tx.gasprice, block.gaslimit));
      bytes32 _seed = sha256(block.blockhash(blockIdx--), ownerOf(totalTicket-idx-1), specialNumber, now);
      uint randNumber = ( uint(_seed) % (totalTicket) );
      //emit RandomNumber(_seed, randNumber);
      if ( !isExistKey(randNumber) ) {

        if ( idx < FirstPrize ) {
          setTokenURI(randNumber, "1");
          ownerOf(randNumber).transfer(betPrice.mul(totalTicket).mul(5).div(100));
          totalPrize += betPrice.mul(totalTicket).mul(5).div(100);
          //emit Sendprize(1, betPrice.mul(totalTicket).mul(5).div(100), ownerOf(randNumber));
          emit ethTransfer(adress(this), ownerOf(randNumber), betPrice.mul(totalTicket).mul(5).div(100));
        }
        if ( idx >= FirstPrize && idx < (FirstPrize+SecondPrize) ) {
          setTokenURI(randNumber, "2");
          ownerOf(randNumber).transfer(betPrice.mul(10));
          totalPrize += betPrice.mul(10);
          //emit Sendprize(2, betPrice.mul(10), ownerOf(randNumber));
          emit ethTransfer(adress(this), ownerOf(randNumber), betPrice.mul(10));
        }
        if ( idx >= (FirstPrize+SecondPrize) && idx < (FirstPrize+SecondPrize+ThirdPrize) ) {
          setTokenURI(randNumber, "3");
          ownerOf(randNumber).transfer(betPrice.mul(5));
          totalPrize += betPrice.mul(5);
          //emit Sendprize(3, betPrice.mul(5), ownerOf(randNumber));
          emit ethTransfer(adress(this), ownerOf(randNumber), betPrice.mul(5));
        }
        if ( idx >= (FirstPrize+SecondPrize+ThirdPrize) && idx < (FirstPrize+SecondPrize+ThirdPrize+FourthPrize) ) {
          setTokenURI(randNumber, "4");
          ownerOf(randNumber).transfer(betPrice.mul(3));
          totalPrize += betPrice.mul(3);
          //emit Sendprize(4, betPrice.mul(3), ownerOf(randNumber));
          emit ethTransfer(adress(this), ownerOf(randNumber), betPrice.mul(3));
        }
        if ( idx >= (FirstPrize+SecondPrize+ThirdPrize+FourthPrize) ) {
          setTokenURI(randNumber, "5");
          ownerOf(randNumber).transfer(betPrice);
          totalPrize += betPrice;
          //emit Sendprize(5, betPrice, ownerOf(randNumber));
          emit ethTransfer(adress(this), ownerOf(randNumber), betPrice);
        }
