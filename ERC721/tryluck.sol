...
contract sampleLoto is ERC721Token, Ownable {
  ...
  function determineWinners() internal returns (uint) {
    ...
    /* 勝者の人数（合計） */
    totalWinner = (totalTicket).mul(85).div(100);
    ...
    while ( idx < totalWinner ) {
      /* 擬似乱数の生成 */
      bytes32 _seed = sha256(block.blockhash(blockIdx--), ownerOf(totalTicket-idx-5), idx, now);
      ...
      if ( !isExistKey(randNumber) ) {
        ...
        ownerOf(randNumber).transfer(betPrice.mul(105).div(100));
        totalPrize += betPrice.mul(105).div(100);
        emit Sendprize(1, betPrice.mul(105).div(100), ownerOf(randNumber));
        /* 勝者を確定 */
